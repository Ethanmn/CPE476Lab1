...
CFLAGS=-g -Wall -Wextra -std=c++11 -MMD `sdl2-config --cflags` `pkg-config --cflags glew gl`
...
LDLIBS=`sdl2-config --libs` `pkg-config --libs glew gl` -lboost_system -lboost_filesystem
